#!/usr/bin/env bash
# Stuck-at actuator fault robustness sweep across three checkpoint types
...
#
# Usage:
#   bash scripts/reinforcement_learning/skrl/run_stuck_at_sweep.sh
#
# Outputs:
#   results/{mappo,ippo_tuned,mappo_act_adv}_stuck_at_sweep_cart.txt
#   results/{mappo,ippo,mappo_act_adv}_stuck_at_sweep_shadow.txt
#   results/Actuation_fault/cart/{mappo,ippo_tuned,mappo_act_adv}_diffusion_stuck_at_sweep_cart.txt
#   results/Actuation_fault/shadow/{mappo,ippo,mappo_act_adv}_diffusion_stuck_at_sweep_shadow.txt
...
ACTUATION_FAULT_DIR="$ISAACLAB_ROOT/results/Actuation_fault"
mkdir -p "$ACTUATION_FAULT_DIR/cart" "$ACTUATION_FAULT_DIR/shadow"
...
run_cart() {
  ...
  conda run -n isaaclab bash "$ISAACLAB_ROOT/isaaclab.sh" -p "$SCRIPT" \
    ...
    --checkpoint "$ckpt" --attack_kind stuck_at --sp_probs $SP_PROB_LIST \
    --num_envs $NUM_ENVS --num_episodes $NUM_EPISODES --headless \
    --out "$ISAACLAB_ROOT/results/${tag}_stuck_at_sweep_cart.txt"
}
...
run_shadow() {
  ...
  conda run -n isaaclab bash "$ISAACLAB_ROOT/isaaclab.sh" -p "$SCRIPT" \
    ...
    --headless \
    --out "$ISAACLAB_ROOT/results/${tag}_stuck_at_sweep_shadow.txt"
}
...
run_cart_diffusion() {
  ...
  conda run -n isaaclab bash "$ISAACLAB_ROOT/isaaclab.sh" -p "$DIFFUSION_SCRIPT" \
    ...
    --attack_kind stuck_at --sp_probs $SP_PROB_LIST --t_start_list $DIFFUSION_T_START_LIST \
    --num_envs $NUM_ENVS --num_episodes $NUM_EPISODES --headless \
    --out "$ACTUATION_FAULT_DIR/cart/${tag}_diffusion_stuck_at_sweep_cart.txt"
}
...
run_shadow_diffusion() {
  ...
  conda run -n isaaclab bash "$ISAACLAB_ROOT/isaaclab.sh" -p "$DIFFUSION_SCRIPT" \
    ...
    --success_threshold 0.1 --reward_scale 2.0 --dist_reward_scale 20.0 \
    --headless \
    --out "$ACTUATION_FAULT_DIR/shadow/${tag}_diffusion_stuck_at_sweep_shadow.txt"
}
...
for i in "${!CART_TAGS[@]}"; do
  ...
  cat "$ISAACLAB_ROOT/results/${CART_TAGS[$i]}_stuck_at_sweep_cart.txt"
  echo ""; echo "=== Cart Double Pendulum: ${CART_TAGS[$i]}_diffusion ==="
  cat "$ACTUATION_FAULT_DIR/cart/${CART_TAGS[$i]}_diffusion_stuck_at_sweep_cart.txt"
done

for i in "${!SHADOW_TAGS[@]}"; do
  echo ""; echo "=== Shadow Hand Over: ${SHADOW_TAGS[$i]} ==="
  cat "$ISAACLAB_ROOT/results/${SHADOW_TAGS[$i]}_stuck_at_sweep_shadow.txt"
  echo ""; echo "=== Shadow Hand Over: ${SHADOW_TAGS[$i]}_diffusion ==="
  cat "$ACTUATION_FAULT_DIR/shadow/${SHADOW_TAGS[$i]}_diffusion_stuck_at_sweep_shadow.txt"
done
