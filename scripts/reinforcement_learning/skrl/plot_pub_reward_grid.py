"""
Publication-ready reward robustness grid (Cart + Shadow-Hand x 5 fault sweeps).

Authored at final IEEE two-column \\textwidth print size, vector PDF output
(+ 300-dpi PNG preview), Okabe-Ito colorblind-safe palette, compact legend
(color = algorithm, linestyle = diffusion on/off), sharex='col' so each
column shows one shared set of x-ticks across both task rows.

Include in LaTeX as:
    \\begin{figure*}[t]
      \\centering
      \\includegraphics[width=\\textwidth]{results/pub_reward_grid.pdf}
      \\caption{...}
    \\end{figure*}
"""
import re
...
def plot_lines(ax, algo_dict, xvals):
    for algo in ALGO_ORDER:
        ...
        for mean, std, ls, marker in [(mean_no, std_no, "-", "o"), (mean_t20, std_t20, ":", "s")]:
            lo = [m - s for m, s in zip(mean, std)]
            hi = [m + s for m, s in zip(mean, std)]
            ax.fill_between(xvals, lo, hi, color=color, alpha=0.12, linewidth=0)
            ax.plot(xvals, mean, color=color, linestyle=ls, linewidth=1.0,
                    marker=marker, markersize=2.2, markeredgewidth=0)
...
fig.supylabel("Reward", fontsize=7)
...
for ext, kw in (("pdf", {}), ("png", {"dpi": 300})):
    out_path = f"results/pub_reward_grid.{ext}"
